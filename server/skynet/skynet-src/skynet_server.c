...
skynet_context_message_dispatch(struct skynet_monitor *sm, struct message_queue *q, int weight) {
	...
	for (i = 0; i < n; i++) {
		...
		if ((i+1) == n && max > 0) {
			uint64_t nn = skynet_mq_length(q);
			if (nn > 0) {
				n += nn;
			}
			max--;
...
static const char *
cmd_priority(struct skynet_context * context, const char * param) {
	if (param[0] == '0') {
		context->priority = false;
		struct message_queue * qu = context->queue;
		if (qu) {
			qu->priority = false;
		}
	} else {
...
uint32_t
skynet_service_key(struct skynet_context * ctx, const char * param) {
