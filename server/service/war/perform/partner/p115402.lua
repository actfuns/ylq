...
function CPerform:TruePerform(oAttack,oVictim,iDamageRatio)
    ...
    local iCnt = mArgs["cnt"] or 3
    local iActionWid = oAttack:GetWid()
    local iWarID = oAttack:GetWarId()
    local iSelectWid
    ...
    for iCurrentCnt=1,iCnt do
        ...
        local iRatio = 100 - 10 * iCurrentCnt
...
function CPerform:XiuZhengMagicTime(oWar,iAttackCnt)
    ...
    if iAddTime > 0 then
        oWar:AddAnimationTime(iAddTime,{skill=self:Type(),mgi=-1,ex="xiuzheng"})
    end
end
