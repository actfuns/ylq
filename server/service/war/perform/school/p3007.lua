...
function CPerform:OnAttacked(oVictim,oAttack,iDamage)
    ...
    local iRatio = mSkillArgs["attack_ratio"] or 500
...
function AttackBack(oVictim,oAttack,oPerform)
    if not oAttack:IsCurrentAction() or not oVictim:IsAttackBack() or oVictim:HasKey("disable") then
        return
    end
    ...
    oActionMgr:WarNormalAttack(oVictim,oAttack,oPerform,100,2)
