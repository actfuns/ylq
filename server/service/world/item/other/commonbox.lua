...
function CItem:TrueUse(oPlayer, iTarget, iAmount)
    ...
    local lFilter = self:FilterSex(oPlayer, lReward)
...
function CItem:FilterSex(oPlayer, lReward)
    local lFilter = {}
    ...
    for _, mItem in ipairs(lReward) do
        ...
        if self:IsEquip(oItem) then
            if self:IsWeapon(oItem) then
                if self:ValidAddWeapon(oPlayer, oItem) then
...
function CItem:ValidAddWeapon(oPlayer, oEquip)
