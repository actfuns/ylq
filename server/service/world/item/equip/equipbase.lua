...
function CItem:Load(mData)
    ...
    for _,mSEData in pairs(mSE) do
        ...
        local oSE = loadskill.NewSkill(iSkill)
...
function CItem:GetPower()
    ...
    if iStoneShape then
        ...
        record.error(string.format("err getpower %s %s %s",iPid, self:ID(), iSchool))
    end
...
function CItem:GetFuWen(sAttr,rDefault)
    self:Dirty()
    rDefault = rDefault or 0
    return self.m_mFuWen[sAttr] or rDefault
end
...
function CItem:PackFuWenInfo()
    ...
    for iPlan, m in pairs(self.m_mFuwenPlan) do
        ...
        mNet["plan"] = iPlan
        local lApply = {}
        for sAttr, iValue in pairs(m["fuwen"] or {}) do
            table.insert(lApply, {key = sAttr, value = iValue})
        end
        mNet["fuwen_attr"] = lApply
        lApply = {}
        for sAttr, iValue in pairs(m["back_fuwen"] or {}) do
            table.insert(lApply, {key = sAttr, value = iValue})
        end
...
function CItem:SendFuWen(oPlayer)
    local mApply = {}
    for sApply,iValue in pairs(self.m_mFuWen) do
        table.insert(mApply,{key=sApply,value=iValue})
    end

    local mBackApply = {}
    local mBackFuwen = self:GetData("back_fuwen",{})
    for sApply,iValue in pairs(mBackFuwen) do
...
function CItem:UseBackFuWen(oPlayer)
    -- local mBackFuwen = self:GetData("back_fuwen",{})
    ...
    -- self.m_mFuWen = {}
    -- self:SetData("back_fuwen",nil)

    -- for sAttr,iValue in pairs(mBackFuwen) do
    --     self:AddFuWen(sAttr,iValue)
    --     if bWield then
    --         oPlayer.m_oEquipMgr:AddApply(sAttr,self:EquipPos(),iValue)
    --     end
    -- end
    -- self:SendFuWen(oPlayer)
...
function CItem:AddFuWenPlan(iPlan, mFuWen)
    self:Dirty()
    local m = {}
    m["fuwen"] = mFuWen or {}
    m["back_fuwen"] = nil
...
function CItem:UnCalFuWen(oPlayer)
    ...
    local iPos = self:EquipPos()
    for sAttr, iValue in pairs(mFuWen) do
        oPlayer.m_oEquipMgr:AddApply(sAttr,iPos,-iValue)
    end
end
