...
function CItem:PreCheckPartner(oPartnerCtrl)
    local iWield = self:GetData("wield")
    local bRefresh = false
    local iPid,iTraceNo = table.unpack(self:GetData("TraceNo",{}))
    if iWield > 0 then
        local oPartner = oPartnerCtrl:GetPartner(iWield)
        if oPartner then
    ...
    local lErrorId = {}
    for iPartnerId, mPlanInfo in pairs(self.m_mPlanInfo) do
        local oPartner = oPartnerCtrl:GetPartner(iPartnerId)
        if oPartner then
...
function CItem:OnUpGrade(oPlayer)
    ...
    if iLevel == 15 then
        global.oPartnerMgr:PushAchieve(oPlayer:GetPid(), "强化等级15符文数量",{value = 1})
    end
end
