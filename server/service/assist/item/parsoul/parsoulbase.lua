...
function CItem:GetPlanList()
    return self:GetData("soul_plan", {})
end

function CItem:IsPlan()
    return table_count(self:GetPlanList()) > 0
end

function CItem:AddSoulPlan(iPlan, iPos)
    local mPlan = self:GetPlanList()
    mPlan[iPlan] = iPos
    self:SetData("soul_plan", mPlan)
    self:GS2CRefreshPartnerSoulInfo()
end

function CItem:RemoveSoulPlan(iPlan, iPos)
    local mPlan = self:GetPlanList()
    mPlan[iPlan] = nil
    self:SetData("soul_plan", mPlan)
    self:GS2CRefreshPartnerSoulInfo()
...
function CItem:CheckUpGrade(oPlayer)
    ...
    if i > iLevel then
        ...
        if iLevel < 15 and i >= 15 then
            global.oAssistMgr:PushAchieve(oPlayer:GetPid(), "15级御灵散件个数", {value = 1})
        end
    end
end
...
function CItem:UnWield(oPlayer, oPartner, iPos)
    ...
    if iTarget == iWield then
        oPartner:UnWieldSoul(iPos, true)
    else
...
function CItem:UnWield1(oPlayer, mData, iWield, iTarget, iPos)
    if mData.answer == 1 then
        ...
        if oTarget then
            oPartner:UnWieldSoul(self:WieldPos(), true)
