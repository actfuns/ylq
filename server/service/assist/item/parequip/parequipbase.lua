...
function CItem:Use(oPlayer, iParId)
    ...
    if self:IsWield() then
        ...
        local iHistory = oPlayer.m_oPartnerCtrl:HistoryWieldEquip()
        self:Wield(oPartner)
        local iNow = oPlayer.m_oPartnerCtrl:CountWieldEquip()
        if iHistory < iNow then
            global.oAssistMgr:PushAchieve(oPlayer:GetPid(), "穿戴符文", {value = iNow - iHistory})
            oPlayer.m_oPartnerCtrl:UpdateWieldEquipAmount(iNow)
        end
