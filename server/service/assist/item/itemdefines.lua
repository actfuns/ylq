...
function ValidParSoul(oPlayer, iSoulType, lSoulPos)
    local oAssistMgr = global.oAssistMgr
    local iPid = oPlayer:GetPid()
    local mSoulType = GetParSoulTypeData(iSoulType)
    if not mSoulType then
        oAssistMgr:BroadCastNotify(iPid, nil, "御灵核心缺失")
        return false
    end
    ...
    for _, m in ipairs(lSoulPos) do
        local mSoulPos = GetParSoulPosData(m.pos)
        if not mSoulPos then
            oAssistMgr:BroadCastNotify(iPid, nil, "御灵部位不存在")
            return false
        end
        if iGrade < mSoulPos.unlock_grade then
            oAssistMgr:BroadCastNotify(iPid, nil, "御灵部位未开放")
            return
        end
        local oItem = oPlayer:HasItem(m.itemid)
        if not oItem then
            oAssistMgr:BroadCastNotify(iPid, nil, "御灵道具不存在")
            return false
        end
        ...
        if oItem:GetSoulType() ~= iSoulType then
            oAssistMgr:BroadCastNotify(iPid, nil, "核心不可穿戴该御灵")
            return false
        end
        if mSoul[m.itemid] then
            oAssistMgr:BroadCastNotify(iPid, nil, "重复御灵")
            return false
        end
        mSoul[m.itemid] = true
        if mAttrType[oItem:AttrType()] then
            oAssistMgr:BroadCastNotify(iPid, nil, "同属性类型的御灵仅可穿戴1件")
            return false
        end
        mAttrType[oItem:AttrType()] = true
    end
    return true
end
