...
function CItem:TrueUse(oPlayer, iTarget, iAmount,mArgs)
    if self:OpenType() == 1 then
        self:OpenGift(oPlayer, iTarget, iAmount,mArgs)
    else
        global.oNotifyMgr:Notify(oPlayer:GetPid(), "非礼包道具")
    end
end

function CItem:OpenType()
    return self:GetItemData()["open_type"]
end

function CItem:OpenGift(oPlayer, iTarget, iAmount,mArgs)
