...
struct aoi_space * 
aoi_create(aoi_Alloc alloc, float dis, void *ud) {
...
inline static bool
is_near(struct aoi_space * space, float p1[3], float p2[3]) {
	return DIST2(p1,p2) < POW2(space->aoi_dis) * 0.25f ;
}
	...
	while (p) {
		...
		if (p->watcher->version != p->watcher_version ||
			...
			*last = next;
			...
			if (distance2 > POW2(space->aoi_dis) * 4) {
				drop_pair(space,p);
				*last = next;
			} else if (distance2 < POW2(space->aoi_dis)) {
...
gen_pair(struct aoi_space * space, struct object * watcher, struct object * marker, aoi_Callback cb, void *ud) {
	...
	if (distance2 < POW2(space->aoi_dis)) {
...
struct aoi_space * 
aoi_new(float dis) {
	return aoi_create(default_alloc, dis, NULL);
}
