...
static void
fields_all_cb(void * p, void * ud){
	lua_State* L = (lua_State*)ud;
	struct _field* fp = (struct _field*) p;

	int len = lua_rawlen(L, -1);
	const char * stype = NULL;
	int itype = _pbcP_type(fp, &stype);

	lua_newtable(L);
	lua_pushinteger(L, fp -> id);
	lua_rawseti(L, -2, 1);
...
static int
_field_all(lua_State *L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	const char * type = luaL_checkstring(L,2);
	lua_newtable(L);
	pbc_fields(env, type, fields_all_cb, (void *)L);
	return 1;	
}

static void
fields_name_cb(void * p, void * ud){
	lua_State* L = (lua_State*)ud;
	struct _field* fp = (struct _field*) p;
	lua_pushstring(L, fp -> name);
	lua_pushinteger(L, fp -> id);
...
static int
_fields_by_name(lua_State * L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	const char * type = luaL_checkstring(L,2);
	lua_newtable(L);
	pbc_fields(env, type, fields_name_cb, (void *)L);
	return 1;
}

static void
fields_id_cb(void * p, void * ud){
	lua_State* L = (lua_State*)ud;
	struct _field* fp = (struct _field*) p;
	lua_pushinteger(L, fp -> id);
	lua_pushstring(L, fp -> name);
...
static int
_fields_by_id(lua_State * L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	const char * type = luaL_checkstring(L,2);
	lua_newtable(L);
	pbc_fields(env, type, fields_id_cb, (void *)L);
	return 1;
}
...
luaopen_protobuf_c(lua_State *L) {
	luaL_Reg reg[] = {
		...
		{"_decode", _decode },
		{"_fields_by_name", _fields_by_name},
		{"_fields_by_id", _fields_by_id},
		{"_field_all", _field_all},
		{"_gc", _gc },
