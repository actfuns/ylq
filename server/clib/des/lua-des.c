...
ldesencode(lua_State *L) {
	...
	if (textsz != 8) {
		luaL_error(L, "Invalid encode text size %d, need 8 bytes", (int)textsz);
	}
	uint8_t buffer[8];
	des_crypt(SK, text, buffer);
...
ldesdecode(lua_State *L) {
	...
	const uint8_t * text = (const uint8_t *)luaL_checklstring(L, 2, &textsz);
	if (textsz != 8) {
		luaL_error(L, "Invalid decode text size %d, need 8 bytes", (int)textsz);
	}
	uint8_t buffer[8];
	des_crypt(SK, text, buffer);
